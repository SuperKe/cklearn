 ...
 * @Describe
 */
class ApiResultAdapter : CallAdapter.Factory() {
        ...
        delegate.enqueue(object : Callback<T> {
            override fun onResponse(call: Call<T>, response: Response<T>) {
                if (response.isSuccessful) {//http status 是200+
                    ...
                    val apiResult = if (response.body() == null) {
                        ApiResult.onError(ApiError.dataIsNull.errorCode, ApiError.dataIsNull.errorMsg)
                    } else {
                        ApiResult.onSuccess(response.body()!!)
                    }
                    callback.onResponse(this@ApiResultCall, Response.success(apiResult))
                } else {//http status错误
                    val failureApiResult = ApiResult.onError(ApiError.httpStatusCodeError.errorCode, ApiError.httpStatusCodeError.errorMsg)
            ...
            override fun onFailure(call: Call<T>, t: Throwable) {
                if (!NetworkUtils.isConnected()) {
                    //没有网络
                    val apiResult = ApiResult.onError(ApiError.unNetWorkException.errorCode, ApiError.unNetWorkException.errorMsg)
                    callback.onResponse(this@ApiResultCall, Response.success(apiResult))
                    return
                }
                val failureApiResult = if (t is ApiException) {//Interceptor里会通过throw ApiException 来直接结束请求 同时ApiException里会包含错误信息
                    ApiResult.onError(t.errorCode, t.errorMsg)
                } else {
                    ApiResult.onError(ApiError.unknownException.errorCode, ApiError.unknownException.errorMsg)
                }
                callback.onResponse(this@ApiResultCall, Response.success(failureApiResult))
            }
        })
